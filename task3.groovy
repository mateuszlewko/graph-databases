...
g.V().has('airport', 'code', 'MUC').inE().outV().has('country', 'PL') \
     ...
     .select('pl', 'de', 'dist').by('code').by('code').by('dist')
...
g.V().has('code', 'WRO').repeat(out()).emit(has('airport', 'country', 'UK')).until(out().has('airport', 'country', 'UK')).out().has('airport', 'country', 'UK').as('out').select('out').by('code').dedup().fold()
...
g.V().has('country', 'PL')                       \
     .has('type','airport')                      \
     .as('airport')            \
     .select('airport', 'airport', 'airport')        \
     .by('code')                                 \
     .by(inE('route').count())                   \
     .by(outE('route').count())                  
